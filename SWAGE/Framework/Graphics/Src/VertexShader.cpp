...
SWAGE::Graphics::VertexShader::~VertexShader()
{
	ASSERT(mVertexShader == nullptr, "VertexShader --- Terminate must be called before destruction.");
	ASSERT(mInputLayout == nullptr, "InputLayout --- Terminate must be called before destruction.");
}

void SWAGE::Graphics::VertexShader::Initialize(const std::filesystem::path& filePath, uint32_t vertexFormat, const char* entryPoint)
{
	// -------------------------------------------
...
void SWAGE::Graphics::VertexShader::Terminate()
{
	SafeRelease(mVertexShader);
	SafeRelease(mInputLayout);
}

void SWAGE::Graphics::VertexShader::Bind() const
{
